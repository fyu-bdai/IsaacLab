...
def x11_check(statefile: Statefile) -> tuple[list[str], dict[str, str]] | None:
    ...
    if __ISAACLAB_X11_FORWARDING_ENABLED is None:
        ...
        if x11_answer.lower() == "y":
            ...
            print(f"[INFO] To disable X11 forwarding, set __ISAACLAB_X11_FORWARDING_ENABLED=0 in {statefile.path}")
        else:
            ...
            print(f"[INFO] To enable X11 forwarding, set __ISAACLAB_X11_FORWARDING_ENABLED=1 in {statefile.path}")
    else:
...
def x11_refresh(statefile: Statefile):
    ...
    statefile.namespace = "X11"
    __ISAACLAB_TMP_XAUTH = statefile.load_variable("__ISAACLAB_TMP_XAUTH")
    if __ISAACLAB_TMP_XAUTH is not None:
        __ISAACLAB_TMP_XAUTH = Path(__ISAACLAB_TMP_XAUTH)
        if __ISAACLAB_TMP_XAUTH.exists():
